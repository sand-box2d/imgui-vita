...
void ImDrawList::ChannelsMerge()
{
    ...
    {
        ImDrawChannel& ch = _Channels[i];
        if (int sz = ch.CmdBuffer.Size) { sceClibMemcpy(cmd_write, ch.CmdBuffer.Data, sz * sizeof(ImDrawCmd)); cmd_write += sz; }
        if (int sz = ch.IdxBuffer.Size) { sceClibMemcpy(_IdxWritePtr, ch.IdxBuffer.Data, sz * sizeof(ImDrawIdx)); _IdxWritePtr += sz; }
    }
...
void ImDrawList::ChannelsSetCurrent(int idx)
{
    ...
    sceClibMemcpy(&_Channels.Data[_ChannelsCurrent].CmdBuffer, &CmdBuffer, sizeof(CmdBuffer)); // copy 12 bytes, four times
    sceClibMemcpy(&_Channels.Data[_ChannelsCurrent].IdxBuffer, &IdxBuffer, sizeof(IdxBuffer));
    _ChannelsCurrent = idx;
    sceClibMemcpy(&CmdBuffer, &_Channels.Data[_ChannelsCurrent].CmdBuffer, sizeof(CmdBuffer));
    sceClibMemcpy(&IdxBuffer, &_Channels.Data[_ChannelsCurrent].IdxBuffer, sizeof(IdxBuffer));
...
ImFont* ImFontAtlas::AddFont(const ImFontConfig* font_cfg)
{
    ...
    if (!new_font_cfg.FontDataOwnedByAtlas)
    {
        ...
        sceClibMemcpy(new_font_cfg.FontData, font_cfg->FontData, (size_t)new_font_cfg.FontDataSize);
    }
...
const ImWchar*  ImFontAtlas::GetGlyphRangesJapanese()
{
    ...
    if (!full_ranges_unpacked)
    {
        ...
        sceClibMemcpy(full_ranges, base_ranges, sizeof(base_ranges));
...
static void stb__lit(const unsigned char *data, unsigned int length)
{
    ...
    sceClibMemcpy(stb__dout, data, length);
    stb__dout += length;
}
